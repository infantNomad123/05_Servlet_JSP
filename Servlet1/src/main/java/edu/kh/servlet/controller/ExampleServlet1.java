...
import jakarta.servlet.http.HttpServlet; //Tomcat 이 제공하는 클래스
...
public class ExampleServlet1 extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		 ...
		 * 
		 * HttpServletRequest
		 * 	- 클라이언트 요청 시 자동 생송되는 겍체
		 * 
		 * - 요청 시 전달된 데이터,
		 *   요청한 클라이언트 정보,
		 *   요청을 유형하게 처리하기 위한 객체 등을 제공
		 *  
		 *
		 *HttpServletResponse
		 * - 클라이언트 요청 시 자동 생성되는 객체
		 * 
		 * - 서버가 클라이언트에세 응답할 수 있는 방법을 제공
		...
		// * 요청에 담긴 특정 파라미터 얻어오기 *
		// ->  String HttpServletRequest.getParameter(" name 속성 값")
		 ...
		 * HTML 코드를 출력             -> 출력 받은 HTML 코드를 해것해서 화면 구성
		 * 
		 * 1)  응답하는 문서(데이터)의 셩식과 문자 인코딩 지정
		 * 
		 * 2)  출력용 스트림 얻어오기
		 * 
		 * 스트림을 이용해서 html 코드 출력하기
		 * 
		 * 
		 * 
		 */
		...
		resp.setContentType("text/html; charset=utf-8"); //	u	nicode encoding 8
