...
<body>
	...
	<table border = "1" style = "border-collapse: collapse;" >
			<thead>
					...
					<%-- c:forEach == JSTL의 향상된 FOR 문 
						items: 순차 접근할 List, qoduf
						 var: 하나씩 꺼네서 저장할 변수 이름  지정
						varStatus: 반복 상태 관련 갹체(몇번 인덱스, 첫바퀴.?, 마지막?)
					--%>
						 ...
						 --%>
								...
								<th>
									<%-- c:if (조건문, else 없음
											...
											not == ! (논리 부정 연산자)
									 --%>
										<c:if test = "${todo.complete }">0 </c:if>
		...
		<!-- JSP에서, Java(EL, JSTL) ,Front(HTML, CSS, JS)  중 Java 가 해석 우선 순위가 높다! 
					(먼저 해석 된다!!)
		--> 
		
		<%-- session scope에 message속성이 있다면 --%>
	<c:if test ="${not empty sessionScope.message } ">
		<script>
		//EL을 이용해서 String 을 출력할때 꼭 양쪽에 ""  추가하기!!
